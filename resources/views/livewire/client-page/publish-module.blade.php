<div>
    <div class="max-w-4xl p-6 bg-white shadow-lg rounded-lg">
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-start space-x-4">
                <span class="w-8 h-8 flex items-center justify-center bg-blue-500 text-white rounded-full font-bold">3</span>
                <p>Berikan nama file zip yang unik, lalu upload melalui FTP.</p>
            </div>
            <div class="flex items-start space-x-4">
                <span class="w-8 h-8 flex items-center justify-center bg-blue-500 text-white rounded-full font-bold">4</span>
                <p>Sistem akan melakukan verifikasi modul secara otomatis.</p>
            </div>
        </div>
    
        <div class="mt-6 flex space-x-4">
            <a href="/template.zip" class="flex items-center px-4 py-2 bg-green-500 text-white rounded-lg shadow hover:bg-green-600">
        ...
        <div class="mt-6 p-4 bg-gray-100 rounded-lg">
            ...
            <div class="flex items-center space-x-2">
                ...
                <button onclick="navigator.clipboard.writeText(this.parentElement.querySelector('span').textContent)" 
    ...
    <div class="max-w-4xl mt-10 p-6 bg-white shadow-lg rounded-lg">
        ...
        <div class="max-h-16 overflow-y-auto border rounded-lg p-4 space-y-3 bg-gray-50">
            @foreach ($changelogs as $log)
                <p class="font-semibold">[{{ $log->created_at->format('d-m-Y H:i') }}] {{ $log->message }}</p>
            @endforeach
        </div>
    </div>
</div>